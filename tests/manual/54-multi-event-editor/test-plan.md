# Test 50 â€” Multi-Event Definition Editor
...
- `message-or-signal-catch.bpmn` â€” IntermediateCatchEvent with Message + Signal definitions
- `multi-throw.bpmn` â€” IntermediateThrowEvent with two Signal definitions
- `multiple-boundary.bpmn` â€” BoundaryEvent with Message + Timer definitions

## Steps

1. Open the Web UI at `https://localhost:7124` â†’ **Editor**.
2. Import `message-or-signal-catch.bpmn`. Select the multi-event catch gateway.
   - **Verify**: Properties panel shows "Definition #1 â€” Message" and "Definition #2 â€” Signal" as labeled boxes.
...
4. Import `multi-throw.bpmn`. Select the element with two Signal definitions.
   - **Verify**: Panel shows "Definition #1 â€” Signal" and "Definition #2 â€” Signal".
