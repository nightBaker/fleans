...
- **Error codes are strings, not ints** — `ActivityErrorState.Code`, `ActivityFailed.ErrorCode`, `ActivityInstanceEntry.ErrorCode`, and `CompensationWalkFailed.ErrorCode` are all `string` (not `int`). Numeric HTTP-like codes are passed as `"400"`, `"500"` etc. BPMN error codes from `<errorCode>` attributes in BPMN XML are already strings (e.g. `"NOT_FOUND"`, `"PAYMENT_FAILED"`). `TransactionOutcomeSet.ErrorCode` and `TransactionOutcomeRecord.ErrorCode` remain `int?` (separate concern). When adding new error codes, always use string literals. Changing from `int` to `string` breaks event store deserialization for existing databases — reset the SQLite file or provision a fresh PG schema when deploying this change.
- **Static web assets cache-busting** — local JS files in `wwwroot/js/` must be referenced via `@Assets["js/filename.js"]` (not plain `"js/filename.js"`) so .NET's `MapStaticAssets()` adds content-hash fingerprints. Without this, browsers cache stale JS and Blazor JS interop calls fail with "is not a function" after JS file updates.
