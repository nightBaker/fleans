...
- **Fleans.Mcp auth pattern**: `Fleans.Mcp` follows the same opt-in `JwtBearer` pattern as `Fleans.Api` — enabled only when `Authentication:Authority` is set (otherwise the MCP endpoint stays anonymous). Default audience is **`fleans-mcp`** (intentionally distinct from `fleans-api`) so MCP access can be revoked independently. A `RequireAuthenticatedUser` fallback policy on `AddAuthorizationBuilder()` guards `MapMcp()`. `app.UseAuthentication()` / `app.UseAuthorization()` are inserted between `MapDefaultEndpoints` (anonymous health) and `MapMcp()` and ONLY when auth is enabled — registering them unconditionally would inject an auth challenge into the no-config dev path. Aspire forwards `Authentication__Authority` / `Authentication__ClientId` env vars to the `fleans-mcp` resource via the existing `auth-authority` / `auth-client-id` parameters (`Fleans.Aspire/Program.cs`). Per-tool scope/role policies are deferred (same staging as the API/Web role work).
