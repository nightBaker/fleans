...
- **Stream-sharding observability (#591 Phase A)**: `ImplicitSubMetricsHostedService` polls every 30s in `Fleans.Api` and `Fleans.WorkerHost` and emits two structured log lines. **EventId 7000 `LogActiveImplicitSubGrainCount`** reports the total + per-type breakdown of active handler-grain activations (filtered to types whose name contains `Handler`). **EventId 7001 `LogPubSubStoreKeyCount`** reports the Redis `PubSubStore/*` key count via `SCAN MATCH` on the connected `IConnectionMultiplexer`. The Redis probe silently skips when `IConnectionMultiplexer` isn't in DI (Memory-store test paths, Aspire dev with Memory streams). Per-iteration failures log as `LogPollFailure` (EventId 7099) at Warning and resume on the next tick. Feeds #591 Phase A's numeric threshold (PubSubStore key count within 1.5× baseline 30 min post-load ⇒ B/C deferred).
