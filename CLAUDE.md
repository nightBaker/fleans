...
- **`ActivityErrorState.Code` is `string`, not `int`** — changed in commit `bc5ffed`. Test assertions must use string literals: `Assert.AreEqual("500", ...)` and `Assert.AreEqual("400", ...)`, not integer literals. When adding error-code assertions to any test file, always use string form. The database column was also migrated from `integer` to `text` in `20260426120000_ChangeErrorCodeToString.cs`.
- **ESP EndEvent visibility parity** — every Event Sub-Process test that asserts the host (`evtSub1`, etc.) is in `CompletedActivities` should also assert its inner EndEvent. The inner EndEvent MUST be completed before the scope-completion check (`CompleteFinishedSubProcessScopes`) can mark the host complete, so its absence from `CompletedActivities` always indicates a persistence bug. Keep the three ESP test classes (error, timer, message) symmetrical in what they assert.
