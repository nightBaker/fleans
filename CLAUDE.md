...
- **Stream-redelivery idempotency.** Every state-mutating grain method reachable from a `[ImplicitStreamSubscription]` handler MUST be idempotent under at-least-once delivery. Current guarantees are a layered guard set: (1) grain-entry stale guard `HasActiveEntry(activityInstanceId)` on `CompleteActivity`/`FailActivity`; (2) domain-entry stale guard `entry.IsCompleted` as defense-in-depth; (3) `joinState.HasFired` on `CompleteActivationCondition`; (4) `entry.IsCompleted || entry.ActivityInstanceId != hostActivityInstanceId` on `CompleteMultiInstanceEarly`; (5) `CompleteConditionSequence` relies on set-by-key projection idempotency; (6) `WorkflowExecution.IsTimerFireStale(timerActivityId, hostActivityInstanceId)` pre-call check on `HandleTimerFired` — mirrors the aggregate's internal regular-timer and ESP-scope-container guards; late reminder fires (post-completion, post-cancel) are logged as warnings (EventId 3034) and silently dropped. Pairs with `TimerCallbackGrain.Cancel`'s swallow-on-failure cleanup per the registration-vs-cleanup asymmetry rule. (7) **Pending-external-event op-id ledger** (`State.PendingOperations` / `State.AppliedOperations`, #657) — the three caller-retried sibling-grain RPCs (`OnChildWorkflowCompleted`/`OnChildWorkflowFailed`/`OnChildEscalationRaised`) take a deterministic op-id and dedup on it: applied → return persisted outcome; in-flight → await existing in-memory TCS; fresh → durably enqueue via `RaiseEvent(PendingEventEnqueued)`. The queue itself is now persisted (survives forced deactivation; rebuilt with fresh TCSes in `OnActivateAsync`), and the drain path emits `PendingEventDrained` (+ `PendingEventApplied` for retried ops). `AppliedOperations` is GC'd on terminal state (guard (2)'s `IsCompleted` covers post-completion retries). The signal path is never retried, so it persists with a fresh-Guid key and no dedup. Caller side: `IGrainFactory.CallWithRetry`; the op-id *string* format lives in the application layer (the domain effect carries only a raw `EscalationInstanceId` Guid). **When adding a new state-mutating method on `IWorkflowInstanceCallback` (or any other interface called from a stream handler), extend this guard set or prove the new path is naturally idempotent.** Silent doubling under redelivery is a production correctness bug, not a test flake.
