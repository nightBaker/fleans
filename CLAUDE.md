...
- **WorkflowInstance partial class layout** — `WorkflowInstance` is split into partial classes by concern. Place new methods in the correct file:
  - `WorkflowInstance.cs` — fields, constructor, `IBoundaryEventStateAccessor` impl, `SetWorkflow`, `GetWorkflowDefinition`, request context/scope utilities
  - `WorkflowInstance.Execution.cs` — core execution loop (`StartWorkflow`, `ExecuteWorkflow`, `ProcessCommands`), command handlers, transition logic (`TransitionToNextActivity`, `CreateNextActivityEntry`), scope management (`CompleteFinishedSubProcessScopes`, `CancelScopeChildren`, `AnyNotExecuting`)
  - `WorkflowInstance.ActivityLifecycle.cs` — completing/failing activities (`CompleteActivity`, `FailActivity`, `CompleteActivityState`, `FailActivityState`, `FailActivityWithBoundaryCheck`), workflow completion (`Complete`), child workflows (`StartChildWorkflow`, `OnChildWorkflowCompleted`, `OnChildWorkflowFailed`, `SetParentInfo`, `SetInitialVariables`), conditions (`CompleteConditionSequence`), event-based gateway cancellation
  - `WorkflowInstance.EventHandling.cs` — external event delivery (`HandleTimerFired`, `HandleMessageDelivery`, `HandleSignalDelivery` + boundary variants), subscription registration (`RegisterMessageSubscription`, `RegisterTimerReminder`, `RegisterSignalSubscription` + boundary variants), `ThrowSignal`
  - `WorkflowInstance.StateFacade.cs` — read-only state accessors (`GetVariable`, `GetVariables`, `GetActiveActivities`, `GetCompletedActivities`, `GetConditionSequenceStates`, `AddConditionSequenceStates`, `SetConditionSequenceResult`)
