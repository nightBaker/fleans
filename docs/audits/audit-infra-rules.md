...
- **dimension:** pluggability
- **severity:** minor
- **signal:** `Fleans.Aspire/Program.cs` does not inject `Fleans__Role` via `WithEnvironment` on the `fleans-core` project, while `charts/fleans/templates/deployment-core.yaml` sets `Fleans__Role=Core`. Search for the absence of `Fleans__Role` on the `fleans-core` / `apiProject` wiring in `src/Fleans/Fleans.Aspire/Program.cs`.
...
## reliability/mcp-tcpsocket-probe
- **dimension:** reliability
- **severity:** minor
- **signal:** `charts/fleans/templates/deployment-mcp.yaml` uses `tcpSocket` for both `livenessProbe` and `readinessProbe`, while every other Fleans deployment template uses `httpGet` with paths `/alive` and `/health`. Search for `tcpSocket` in `charts/fleans/templates/deployment-mcp.yaml` where the other deployment templates use `httpGet`.
- **remediation:** Replace the `tcpSocket` probes with `httpGet` probes on port `mcp` (5200) using paths `/alive` (liveness) and `/health` (readiness), matching the pattern in `deployment-core.yaml:44-55`. The MCP service registers these endpoints via `app.MapDefaultEndpoints()` (`src/Fleans/Fleans.Mcp/Program.cs:35`). See issue #487.
